...
app.command('/hello', async ({ command, ack, say, client }) => {
  ...
  try {
    ...
    const parentMessage = await client.chat.postMessage({
      ...
      text: `<@${command.user_id}>, this is the output of the experiment:`,
      blocks: [
        {
          type: 'section',
          text: {
    ...
    const imagePath = path.join(__dirname, '../static/images/09-02-2025_18-01-41.372229_SD_G[9.5]_S[816631]-300x300.png');

    // Upload the image to Slack in the thread
    const result = await client.files.uploadV2({
      channel_id: command.channel_id,
      thread_ts: parentMessage.ts, // This puts the file in the thread
      file: fs.createReadStream(imagePath),
      filename: '09-02-2025_18-01-41.372229_SD_G[9.5]_S[816631]-300x300.png',
      title: 'Generated Image'
    });

    // Add a message in the thread referencing the image
    ...
    //       text: {
    //         type: 'mrkdwn',
    //         text: `Here's the generated image: <${result.files[0].permalink}|View full image>`
    //       }
    //     }
    //   ]
    // });
